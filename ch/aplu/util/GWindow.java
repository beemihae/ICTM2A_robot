 ...
 * Tranformation: px = px( wx ), py = py( wy )
...
public class GWindow extends JFrame
{
  ...
  private final Color DEFAULT_BACKGROUNDCOLOR = Color.white;
...
//    System.out.println( e.toString() );
    ...
    if (e.getID() == WindowEvent.WINDOW_CLOSING)
    {
      ...
      if (_offG2D != null)
        _offG2D.dispose();
      if (_panel != null && _panel.getGraphics() != null)
        _panel.getGraphics().dispose();
      System.exit(0);  // Must force to terminate here
    }
    else
  ...
  /**
   * Draw the last loaded image into offscreen buffer
   * (x, y) coordindates of of upper left corner.
  ...
  public void drawImage(int x, int y, boolean lowerLeft)
  {
    ...
    if (lowerLeft)
      at.translate(x, y - _imageHeight);
    else
      at.translate(x, y);
    _offG2D.drawImage(_image, at, _panel);
  }
  ...
  void drawRectangle(double width, double height, boolean fill)
  {
    ...
    int ulx = _toUserX(_xCurrent - width / 2);
    int uly = _toUserY(_yCurrent + height / 2);
               ...
               boolean fill)
  {
    ...
    Arc2D.Double arc =
       new Arc2D.Double(_toUserX(_xCurrent - radius),
                        _toUserY(_yCurrent + radius), _toUserWidth(2 * radius),
                        _toUserHeight(2 * radius), startAngle, extendAngle,
                           Arc2D.OPEN);
    if (fill)
      _offG2D.fill(arc);
    else
  ...
  void showImage(String imagePath, double x, double y)
  {
    Image image = Toolkit.getDefaultToolkit().getImage(imagePath);
    loadImage(image);
    drawImage(_toUserX(x), _toUserY(y), true);
    if (_doRepaint)
      _panel.repaint();
  }

  void showImage(URL imageUrl, double x, double y)
  {
    Image image = Toolkit.getDefaultToolkit().getImage(imageUrl);
    loadImage(image);
    drawImage(_toUserX(x), _toUserY(y), true);
