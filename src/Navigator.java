...
public class Navigator implements WaypointListener
{
    ...
    * @param pilot
    */
   public Navigator(MovePilot pilot)
   {
      this(pilot, null, null);
   }
    ...
    */
   public Navigator(MovePilot pilot, PoseProvider poseProvider, Pose startPose)
   {
      _pilot = pilot;
      ...
      if (startPose == null) {
    	  _pose = new Pose();
      } else _pose = startPose;
      this.poseProvider.setPose(_pose);
      _radius = 0;  //(_pilot instanceof ArcMoveController ? ((ArcMoveController) _pilot).getMinRadius() : 0);
      _nav = new Nav();
      _nav.setDaemon(true);
      _nav.start();
   }
...
//      }
   ...
   public void addWaypoint(Waypoint aWaypoint, boolean first)
   {
	   if (first) {
		   _path.add(0, aWaypoint);
	   }
   }
   /**
   ...
   public void addWaypoint(float x, float y, boolean first)
   {
      addWaypoint(new Waypoint(x, y), first);
   }
...
//             RConsole.println("NAV calling Listeners");
  ...
  private void checkNextWayPoint() {
	   while (_path.size()> 1) {
    	  if (Math.abs(destinationRelativeBearing - _pose.relativeBearing(_path.get(1)))<5f) {
    		  System.out.println("Waypoint "+_path.get(0)+" removed because angle too small");
    		  _path.remove(0);
    		  _destination = _path.get(0);
    	  }
    	  else if ((_pose.distanceTo(_destination))<allowableRemoveDistance){
    		  System.out.println("Waypoint "+_path.get(0)+" removed because distance too small");
    		  _path.remove(0);
    		  _destination = _path.get(0);
    		  destinationRelativeBearing = _pose.relativeBearing(_destination);
    	  }
